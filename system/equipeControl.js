 ...
 */
export async function criaEquipe(interaction) {
  equipeTemp = [];
...
export async function concluiCriacao(interaction) {
  equipeTemp = [];
  const team = await procuraEquipePorLider(
    interaction.user.id,
    interaction.guildId
  );
  if (team) {
...
/*
export async function equipeConcluida(interaction) {
  const level = await Level.findOne({
    userId: interaction.user.id,
    guildId: interaction.guildId,
  });
  if (level) {
    ...
    const team = await Team.findOne({
      leaderId: level.teamLeaderId,
      guildId: interaction.guildId,
    });
    if (team) return team.finished;
  } else {
    return;
  }
  return;
...
*/
export async function deletaEquipe(idLider, guildId) {
  ...
  await Team.findOneAndDelete(query).then(console.log("Deletada com sucesso"));
}
 ...
 */
export async function saiEquipe(interaction) {
 ...
 * @param {ButtonInteraction} interaction
 */
export async function adicionaEquipe(interaction, lider) {
  ...
  if (user) {
    ...
    const team = await procuraEquipePorLider(lider.id, interaction.guildId);
    if (team) {
      const integrantes = team.componentIds;
...
export async function exibeEquipe(interaction) {
  const team = await procuraEquipePorLider(
    interaction.user.id,
    interaction.guildId
  );
...
export async function renomeiaEquipe(interaction, nome) {
  const team = await procuraEquipePorLider(
    interaction.user.id,
    interaction.guildId
  );
 ...
 * @param {ChatInputCommandInteraction} interaction
 */
export async function confereSeJaTemEquipe(interaction) {
 ...
 * @param {ChatInputCommandInteraction} interaction
 */
export async function procuraEquipeDoUsuario(interaction) {
  try {
    const level = await Level.findOne({
      userId: interaction.user.id,
      guildId: interaction.guildId,
    ...
    if (level) {
      ...
      const teamFound = await procuraEquipePorLider(
        level.teamLeaderId,
        interaction.guildId
      );
      if (teamFound) {
...
export async function procuraEquipePorLider(leaderId, guildId) {
  try {
